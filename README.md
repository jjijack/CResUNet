...
2. 运行`run.py`进行训练，输出结果位于`results`目录下，包含最佳模型参数、loss曲线、以及每次性能提升后部分时刻的展示。
...
3. `inference_demo.ipynb`中提供了读取`results/best_model.pth`并输出任一时刻智能订正结果的范例。注意每次运行前会清空`results`目录，因此获得满意的训练结果后建议首先复制备份。